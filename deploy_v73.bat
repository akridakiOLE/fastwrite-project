...
echo.
echo  v73 = DIORTHOSI KEIMENOU ANDROID + SKLIRYNSI TOU ELEGXOU "egkatestimeni;"
echo.
echo        1) EGKATASTASI DEN einai SYNTOMEFSI (A440, 1/9/2026, metrimeno).
echo           I v72 ta edine os isodynama me "i" - kai i karta antifaske me
echo           to idio to FAQ tis efarmogis. To entopise o Stavros se Brave.
echo           Tora: mono "Egkatastasi efarmogis", kai riti proeidopoiisi oti
echo           i Syntomefsi vazei to sima tou browser pano sto eikonidio.
echo.
echo        2) O elegxos "einai idi egkatestimeni;" rotaei MONO standalone.
echo           Ta minimal-ui/fullscreen apantoun "nai" se aplo tab se kapoious
echo           browsers kinitou - kai i protropi pethaine gia panta. Epipleon
echo           kamia MONIMI eggrafi apo parodiki endeixi: mono to appinstalled.
echo.
...
if errorlevel 1 goto failed
git add tests\install_ui.test.mjs deploy_v73.bat commit_v73.txt
if errorlevel 1 goto failed
git commit -F commit_v73.txt
if errorlevel 1 goto failed
...
echo   1) Katharise ta dedomena tou site i anoixe idiotiki periigisi.
echo   2) Anoixe fastwrite.tech/kostometro/ - i karta petagetai moni tis.
echo   3) Sto vima 2 prepei na leei MONO "Egkatastasi efarmogis".
echo      Apo kato, se prasino plaisio: "Mi dialexeis Syntomefsi...".
echo   4) Kane tin egkatastasi kai koita to EIKONIDIO stin arxiki othoni:
echo      prepei na einai KATHARO, xoris to sima tou browser pano tou.
echo   5) Anoixe to apo to eikonidio: xoris grammi diefthinseon.
echo   6) Ekdosi sto Menou kato: v73
echo.
echo ================================
echo.
pause
...
exit /b 1
:lo
echo    Epezise PALIOS deiktis (km-v72, v72 sto version.json, 'inst' sto SCREENS i minimal-ui).
goto failed
:nm

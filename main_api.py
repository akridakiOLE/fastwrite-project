...
def list_pending_documents():
    docs = db.list_documents(status="pending_review")
    for d in docs:
        if d.get("result_json"):
            try: d["result_data"] = json.loads(d["result_json"])
            except: d["result_data"] = None
...
def serve_review_page(doc_id):
    ...
    # Extract page number from file_path
    page_num = 1
...
<title>Εγκριση — %(filename)s</title>
<style>
...
// ── Init ──────────────────────────────────────────────────────────────────────
(async function init() {
  // Load line positions from pdfplumber
...
function renderTable(key, rows) {
  ...
  const thead = document.querySelector('#li-thead tr');
  thead.innerHTML = cols.map(c => '<th>' + c + '</th>').join('') + '<th style="width:30px"></th>';
...
// ── Canvas highlight ──────────────────────────────────────────────────────────
let linePositions = [];  // [{top_pct, bottom_pct}, ...]

async function loadLinePositions() {
  try {
    const res = await fetch('/api/documents/' + DOC_ID + '/line-positions');
...
function drawHighlight(ri) {
  ...
  if (linePositions.length > ri) {
    // Use exact positions from pdfplumber
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'Escape')    { curRow = -1; document.querySelectorAll('#li-tbody tr').forEach(r=>r.classList.remove('row-hl')); drawHighlight(-1); updateNavLabel(); }
});
...
function collectData() {
  ...
  // Arrays from LINE_DATA (updated via td edit)
  Object.keys(LINE_DATA).forEach(key => {
    const rows = [];
    document.querySelectorAll('#li-tbody tr').forEach(tr => {
      const row = {};

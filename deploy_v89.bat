...
echo.
echo  v89 = "ENIMEROTHIKE . v89" STIN KAMERA, MIA FORA ANA EKDOSI
echo.
echo        25/9: protasi Stavros - o xristis na vlepei oti irthe nea ekdosi.
echo        Oxi stin proti egkatastasi. 3 defterolepta, fevgei kai me aggigma.
echo        To "ti neo" erxetai apo to version.json (pedio note).
...
if errorlevel 1 goto nm
findstr /c:"\"v\": \"v89\"" site\kostometro\version.json >nul
if errorlevel 1 goto nm
findstr /c:"v89" site\kostometro\app.js >nul
if errorlevel 1 goto nm
...
if errorlevel 1 goto failed
git add deploy_v89.bat commit_v89.txt
if errorlevel 1 goto failed
git commit -F commit_v89.txt
if errorlevel 1 goto failed
...
echo   1) Anoixe to Kostometro - stin kamera: "Enimerothike . v89" gia 3 defterolepta
...
exit /b 1
:lo
echo    Epezise PALIOS deiktis (km-v88, pk-v4, source link, AIza).
goto failed
:nm

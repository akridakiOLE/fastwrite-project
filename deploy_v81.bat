...
echo.
echo  v81 = I AFTOLEXEI APANTISI TIS GOOGLE STIS RYTHMISEIS
echo.
echo        22/9: i KOKKINOY TAVERNA "den vrethikan posa" 3 fores, xoris na
echo        xeroume TI apantise i Google. Tora grafetai afto-lexei.
echo        Episis: to keimeno mazevetai apo OLA ta meri tis apantisis
echo        (os ti v80 mono to proto) kai i lista [{...}] diavazetai.
echo.
...
if errorlevel 1 goto nm
findstr /c:"km-v81" site\kostometro\sw.js >nul
if errorlevel 1 goto nm
findstr /c:"\"v\": \"v81\"" site\kostometro\version.json >nul
if errorlevel 1 goto nm
findstr /c:"v81" site\kostometro\app.js >nul
if errorlevel 1 goto nm
echo    OK - 14 deiktes.
...
echo [4/5] Souites + 88 apodeixeis metallaxis, MIA TI FORA...
...
if errorlevel 1 goto failed
git add deploy_v81.bat commit_v81.txt
if errorlevel 1 goto failed
git commit -F commit_v81.txt
if errorlevel 1 goto failed
...
echo   1) Anoixe to Kostometro, Menou kato: v81
...
exit /b 1
:lo
echo    Epezise PALIOS deiktis (km-v80, v80, 30000, palio generationConfig i sw.js xoris ekdosi).
goto failed
:nm

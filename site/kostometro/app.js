...
(function () {
  ...
  /* ══ v75 · Ο ΦΡΟΥΡΟΣ ΤΟΥ ΔΙΠΛΟΥ ΛΟΓΑΡΙΑΣΜΟΥ ΣΤΟ iPHONE (20/9/2026) ════
     ...
     Ο ΚΙΝΔΥΝΟΣ: στο iPhone το WebKit μπορεί να δώσει στην εγκατεστημένη
     εφαρμογή ΧΩΡΙΣΤΗ αποθήκευση από το Safari. Όποιος γραφτεί πρώτα
     στο Safari και εγκαταστήσει μετά, ανοίγει μια ΑΔΕΙΑ εφαρμογή — και
     στη σιωπή φτιάχνει ΔΕΥΤΕΡΟ λογαριασμό. Τα τιμολόγια του πρώτου
     δεν χάνονται — αλλά δεν ξέρει ότι υπάρχουν, και ποτέ δεν τα ζητάει.

     ΓΙΑΤΙ Η ΣΥΝΘΗΚΗ ΕΙΝΑΙ ΑΣΦΑΛΗΣ: για να υπάρξει εγκατεστημένη
     εφαρμογή, κάποιος πέρασε πρώτα από το Safari. Άρα «εγκατεστημένη
     + τίποτα τοπικά» είναι ακριβώς η στιγμή του κινδύνου.

     ⚠ ΔΕΝ μπλοκάρει κανέναν. Ο πραγματικά νέος χρήστης — αυτός που
     εγκατέστησε ΠΡΙΝ γραφτεί, όπως τον σπρώχνει η κάρτα — διαβάζει
     μία πρόταση παραπάνω και πατάει «Ξεκινώ τώρα» όπως πριν.

     ⚠ ΑΠΟΦΑΣΗ: η προειδοποίηση ΜΟΝΟ σε iPhone. Στο Android το WebAPK
     μοιράζεται την αποθήκευση με τον Chrome — εκεί η κατάσταση δεν
     προκύπτει ποτέ από σφάλμα, και το μήνυμα θα ήταν σκέτος θόρυβος. */
  function accWarn() {
    var w = el('acc-warn');
    if (!w) { return; }
    var risky = instPlatform() === 'ios' &&
                instStandalone() &&
  ...
  /* 🔴 v49 — Ο ΦΡΑΓΜΟΣ ΤΟΥ ΚΛΕΙΔΩΜΑΤΟΣ, ΣΕ ΕΝΑ ΣΗΜΕΙΟ.
     ...
     ⚠ ΤΟ ΣΧΟΛΙΟ ΠΟΥ ΗΤΑΝ ΕΔΩ ΕΛΕΓΕ ΨΕΜΑΤΑ (διορθώθηκε 7/9/2026, εύρημα
     Stavros). Έλεγε ότι «οι ρυθμίσεις μένουν ανοιχτές επίτηδες, εκεί ζουν οι
     12 λέξεις μου και ο μηδενισμός». Στην πράξη το `s-settings` ζει ΜΕΣΑ στο
     `s-menu`, που είναι φραγμένο — άρα οι ρυθμίσεις ΔΕΝ ανοίγουν σε
     κλειδωμένη συσκευή. **ΚΑΙ ΕΤΣΙ ΠΡΕΠΕΙ.** Οι 12 λέξεις δεν είναι «τα
     δεδομένα αυτού του κινητού»: είναι το κλειδί ΟΛΟΚΛΗΡΟΥ του λογαριασμού,
     για πάντα, και για ό,τι προστεθεί στο μέλλον. Συσκευή που ο ιδιοκτήτης
     έβγαλε εκτός λειτουργίας (π.χ. επειδή του την έκλεψαν) δεν επιτρέπεται να
     τις δείχνει σε όποιον μπορεί να ξεκλειδώσει την οθόνη της — θα του έδινε
     μόνιμη πρόσβαση, ακόμα κι αφού ο ιδιοκτήτης πάρει πίσω τη σκυτάλη.
     🔴 ΜΗΝ «ΔΙΟΡΘΩΣΕΙΣ» ΑΥΤΟ ΒΓΑΖΟΝΤΑΣ ΤΟ s-menu ΑΠΟ ΤΗ ΛΙΣΤΑ. Ο δρόμος του
     ιδιοκτήτη είναι ένας και είναι το μοντέλο του πορτοφολιού: ενεργοποιεί τη
     συσκευή με τις 12 λέξεις, και ΜΕΤΑ έχει ρυθμίσεις και μηδενισμό. Όποιος
     δεν τις ξέρει πάει από την ανάκτηση δύο μισών (Α300 §2, 6/9), που δίνει
     ΝΕΕΣ 12 λέξεις μετά από ταυτοποίηση — όχι από πλαϊνή πόρτα εδώ. */
  ...
  function whoMode(m) {
    ...
    el('who-findbar').hidden = (m !== 'list');   // v45 — η αναζήτηση ανήκει στη λίστα
  ...
  function amountsBlock(r, opts) {
    ...
    if (!sug && localStorage.getItem(LS.key) && empty) {
      ...
      if (!navigator.onLine) {
        ...
        /* v86 · KM-KLEIDI-HALT — εδώ ο χρήστης χρειάζεται τον οδηγό περισσότερο από
           οπουδήποτε: το συνηθέστερο αίτιο είναι άκυρο ή λάθος κλειδί. */
        var guide = document.createElement('a');
        guide.className = 'pay-link';
        guide.href = '/kostometro/kleidi/';   // v87: ίδιο παράθυρο
        guide.textContent = 'Οδηγός κλειδιού ›';
        busy.appendChild(guide);
      } else if (aiPay) {
  ...
  /* 🔴 v66 (19/9/2026) — Η ΣΥΣΤΑΣΗ ΓΡΑΦΕΤΑΙ ΤΗ ΣΤΙΓΜΗ ΤΗΣ ΕΓΓΡΑΦΗΣ, ΟΧΙ ΤΟΥ
     ΠΡΩΤΟΥ ΑΝΟΙΓΜΑΤΟΣ (κανόνας Α400 §Δ, 4/9/2026).
     Ως τη v65 ο έλεγχος ήταν «if (!LS.src)»: το ?ref= διαβαζόταν ΜΟΝΟ σε
     ολοκαίνουργια συσκευή. Μετρημένο 17/9/2026 στη δοκιμή του Stavros —
     άνοιξε τον σύνδεσμο σε υπολογιστή που είχε ήδη km_source, και η σύσταση
     ΔΕΝ γράφτηκε. Το πραγματικό σενάριο μετά τη διανομή είναι χειρότερο:
     επιχείρηση βλέπει τη διαφήμιση, ανοίγει το Kostometro, ΔΕΝ γράφεται· δύο
     βδομάδες μετά παίρνει τον σύνδεσμο ενός φίλου και γράφεται — η σύσταση
     του φίλου χανόταν ΣΙΩΠΗΛΑ, χωρίς κανείς να το μάθει ποτέ.
     Ο κανόνας μένει ακέραιος: όσο ΔΕΝ υπάρχει λογαριασμός, το τελευταίο
     ?ref= κερδίζει· μόλις γίνει η εγγραφή, τίποτα δεν αλλάζει αναδρομικά.
     ⚠ Απομονωμένο ΕΠΙΤΗΔΕΣ σε καθαρή συνάρτηση: μέσα στο boot() δεν
     μετριόταν με τεστ, και ακριβώς εκεί κρύφτηκε το σφάλμα δύο μέρες.
     Επιστρέφει τι πρέπει να γραφτεί στο km_source, ή null = «μην αγγίξεις». */
  function refCaptureSrc(search, registered, current) {
  ...
  /* Πόσο αναλυτικά φαίνεται η στιγμή της εγγραφής.
     Ημέρα, ημερομηνία και ώρα σε ΚΑΘΕ γραμμή — και όπου υπάρχει
     συγκατάθεση, δίπλα στο email. Απόφαση Stavros 19/9/2026 μετά από
     ένσταση (Α320). Αυτό που ΔΕΝ αλλάζει: το email δεν φεύγει ποτέ από τη
     βάση χωρίς ρητή συγκατάθεση — ο φρουρός ζει στο SQL. */
  var MERES = ['Κυρ','Δευ','Τρί','Τετ','Πέμ','Παρ','Σάβ'];
  function refWhen(v){
    var d = String(v || '');
    /* v70 — ημέρα/ημερομηνία/ώρα σε ΚΑΘΕ γραμμή (απόφαση Stavros 19/9/2026).
       Ο τύπος με τους 10 χαρακτήρες μένει ως εφεδρεία: αν ποτέ ο server
       ξαναστείλει σκέτη ημερομηνία, η οθόνη δεν θα δείξει «Invalid Date». */
  ...
  function aiRead(rec, key) {
    ...
    /* v79 · KM-AI-FALLOVER — ΥΠΕΡΦΟΡΤΩΜΕΝΟ ΜΟΝΤΕΛΟ → ΤΟ ΕΠΟΜΕΝΟ, ΑΜΕΣΩΣ.
       ...
       μόνη της (15″ → 120″). */
    var overErr = null;
    /* v80 · KM-AI-DEEP — Η ΧΑΜΗΛΗ ΣΚΕΨΗ ΜΟΝΟ ΣΤΗΝ ΠΡΩΤΗ ΠΡΟΣΠΑΘΕΙΑ.
       Μετρήθηκε 22/9/2026 20:51 με τη v79: η ΚΟΚΚΙΝΟΥ ΤΑΒΕΡΝΑ (θερμικό χαρτί,
       λοξή φωτογραφία, «TOTAL € 37.50» ορατό ακόμα και στη μικρογραφία) πήρε
       ΤΡΕΙΣ απαντήσεις «κανένα ποσό» — με χαμηλή σκέψη σε όλες. Το ίδιο κλειδί
       διάβασε το GREEN GARDEN δίπλα της. Η v78 έκανε τα εύκολα γρήγορα και τα
       δύσκολα χειρότερα από τη v77: λιγότερο από αυτό που υπήρχε.
       Πρώτη προσπάθεια = χαμηλή σκέψη (τα εύκολα σε ~3″). Από τη δεύτερη και
       μετά = η κανονική σκέψη του μοντέλου, όπως ως τη v77. */
    var deep = ((rec && rec.aiTry) || 0) >= 1;
    function step(i) {
      if (i >= list.length) {
      ...
      return aiOnce(rec, key, list[i], deep || !!AI_NOTHINK[list[i]]).then(function (out) {
        ...
        try {
          localStorage.setItem(LS.aiMs, aiStamp() + ' · ' +
            ...
            ((deep || AI_NOTHINK[list[i]]) ? ' · σκέψη κανονική' : ' · σκέψη χαμηλή'));
        } catch (e3) {}
        return out;
      }).catch(function (err) {
        ...
        if (err && err.status === 400 && !deep && !AI_NOTHINK[list[i]]) {
  ...
  function whoChrome(on) {
    ...
    if (d) { d.hidden = !on; }
    if (t) { t.hidden = !on; }
    /* v59 — και το ανάποδο: η «Επιστροφή» υπάρχει ΜΟΝΟ σε διόρθωση. */
    if (el('who-back')) { el('who-back').hidden = on; }
  }
  /* v59 · «‹ ΕΠΙΣΤΡΟΦΗ» ΑΠΟ ΤΗ ΔΙΟΡΘΩΣΗ — εύρημα Stavros 11/9/2026.
     Ως τη v58 η s-who σε διόρθωση δεν είχε κανένα κουμπί εξόδου: στο
     Android μόνο το «πίσω» του κινητού (που ο χρήστης δεν ξέρει ότι πρέπει
     να πατήσει), στο iPhone ΤΙΠΟΤΑ — ο μόνος δρόμος ήταν να μετονομάσεις
     και να πατήσεις αναίρεση (Α440, 4/9: κάθε οθόνη θέλει δρόμο προς τα έξω).
     Το κουμπί είναι ένα απλό [data-back] στο index.html: ΙΔΙΟΣ μηχανισμός
     με το «πίσω» του κινητού, που επαληθεύτηκε στη συσκευή του Stavros
     11/9. Δεύτερος, δικός του χειριστής δοκιμάστηκε και ΔΕΝ διέφερε σε
     τίποτα μετρήσιμο (μετάλλαξη Μ3, τεστ 70-72 πράσινα και με τους δύο) —
     άρα δεν μπαίνει. Δεν αλλάζει καμία εγγραφή· η φόρμα του τιμολογίου
     κρατάει ό,τι είχε γραφτεί (τεστ 71). */
  ...
  /* 🔴 ΕΠΙΣΤΡΕΦΕΙ true ΜΟΝΟ ΑΝ ΑΛΛΑΞΕ ΚΑΤΙ (υιοθέτησε ή μετανάστευσε).
     Βρέθηκε 6/9 από το τεστ 33: με «true και όταν όλα ήταν ήδη εντάξει», η
     γραμμή του v48 έκανε ΔΕΥΤΕΡΟ κατέβασμα σε κάθε άνοιγμα κάθε συσκευής —
     διπλή δουλειά και κούρσα με τον κανονικό συγχρονισμό. Το «τίποτα δεν
     άλλαξε» δεν είναι επιτυχία που δικαιολογεί ενέργεια. */
  function ensureLock() {
  ...
  function wordsAccepted() {
    ...
    /* 🔴 v52 — Η «ΑΛΛΑΓΗ ΛΕΞΕΩΝ» ΔΕΝ ΕΙΝΑΙ ΝΕΟΣ ΛΟΓΑΡΙΑΣΜΟΣ.
       Ως τη v51 το mode 'rotate' έπεφτε εδώ και έφτιαχνε ΚΑΙΝΟΥΡΙΟ Κ και
       ΚΑΙΝΟΥΡΙΟ folder_id — δηλαδή άδειο λογαριασμό δίπλα στον παλιό, με τα
       τιμολόγια να μένουν κλειδωμένα πίσω. Γι' αυτό το κουμπί είχε αφαιρεθεί
       (v39) αντί να διορθωθεί. Με τη βάση Κ (Η.13) η σωστή πράξη είναι άλλη:
       ΙΔΙΟ Κ, ΙΔΙΟΣ ΦΑΚΕΛΟΣ, νέα κλειδαριά από πάνω. */
    if (wordsMode === 'rotate') { return rotateAccepted(); }
    el('w-go').disabled = true;
  ...
  /* ══ v52 · ΑΛΛΑΓΗ 12 ΛΕΞΕΩΝ (Α300 §3 βήμα 4) ══════════════════════
     Νέες λέξεις → νέα κλειδαριά που κλειδώνει το ΙΔΙΟ Κ → ο server σβήνει
     την παλιά στην ΙΔΙΑ πράξη (`replace`). Καμία φωτογραφία, κανένα
     τιμολόγιο, κανένα byte δεδομένων δεν ξαναγράφεται: αλλάζει μόνο η
     κλειδαριά, ~60 bytes. Είναι και η τελευταία πράξη της ΑΝΑΚΤΗΣΗΣ
     (Α320, 6/9), γι' αυτό χτίζεται πρώτη.
     🔴 Η ΣΕΙΡΑ ΤΩΝ ΤΟΠΙΚΩΝ ΕΓΓΡΑΦΩΝ ΔΕΝ ΕΙΝΑΙ ΤΥΧΑΙΑ. Γράφουμε ΠΡΩΤΑ τις
     λέξεις και μετά την κλειδαριά. Αν κάτι σταματήσει ενδιάμεσα, ο χρήστης
     κρατάει στο χαρτί τις λέξεις που ΙΣΧΥΟΥΝ (ο server έχει ήδη τη νέα
     κλειδαριά) και ξανασυνδέεται. Με την αντίστροφη σειρά θα κρατούσε
     νεκρές λέξεις χωρίς να το ξέρει — και αυτό δεν επιστρέφει. */
  function rotateAccepted() {
    var words = pendingWords;
    var b = el('w-go'), e = el('w-err');
    var kHex = localStorage.getItem(LS.kkey);
    var oldLock = localStorage.getItem(LS.lock);
    function stop(msg) {
      b.disabled = false; b.textContent = 'Συνέχεια';
      e.textContent = msg; e.hidden = false;
    }
    /* Χωρίς Κ ή χωρίς κλειδαριά = λογαριασμός v46 που δεν έχει μεταναστεύσει.
       Η αλλαγή λέξεων εκεί ΘΑ έκανε τα δεδομένα αδιάβαστα (το κλειδί έβγαινε
       από τις λέξεις). Δεν προχωράει — και το λέει. */
    if (!kHex || !oldLock) {
      stop('Αυτή η συσκευή δεν έχει περάσει ακόμα στη νέα βάση κλειδιών. Άνοιξε την εφαρμογή μία φορά με δίκτυο και ξαναδοκίμασε.');
      return;
    }
    b.disabled = true; b.textContent = 'Αλλάζει την κλειδαριά…';
    e.hidden = true;
    var rawK = kmHexToBytes(kHex);
    Promise.all([kmDeriveLock(words), kmDerive(words)]).then(function (dl) {
        ...
        return kmFetch('lock', {
          ...
          body: JSON.stringify({
            ...
            /* 🔴 ΚΛΕΙΝΕΙ ΚΑΙ ΤΗΝ ΕΝΑΛΛΑΚΤΙΚΗ ΤΑΥΤΟΤΗΤΑ ΤΟΥ ΛΟΓΑΡΙΑΣΜΟΥ.
               ...
               κωδικό της ΠΡΩΤΗΣ κλειδαριάς· σε λογαριασμό που μετανάστευσε από
               v46, με τον παλιό κωδικό από τις λέξεις. Και στις δύο περιπτώσεις
               μένει ζωντανός μετά την αλλαγή αν δεν τον γράψουμε από πάνω — και
               τότε ΟΙ ΠΑΛΙΕΣ ΛΕΞΕΙΣ ΣΥΝΕΧΙΖΟΥΝ ΝΑ ΜΠΑΙΝΟΥΝ, από την πλάγια
               πόρτα, χωρίς κλειδαριά. Στέλνουμε τον κωδικό της ΝΕΑΣ κλειδαριάς:
               ό,τι κι αν ήταν εκεί μέσα, σβήνεται.
               ⚠ Μετρήθηκε 7/9 με μετάλλαξη: με τον κωδικό των ΛΕΞΕΩΝ (kmDerive)
               ο φρουρός έμενε πράσινος χωρίς να δοκιμάζει τίποτα. */
          ...
          if (!j || !j.ok) { throw new Error('Ο διακομιστής δεν δέχτηκε την αλλαγή. Δοκίμασε ξανά.'); }
          /* Από εδώ και κάτω η αλήθεια είναι ΣΤΟΝ SERVER. Πρώτα οι λέξεις. */
          localStorage.setItem(LS.words, words.join(' '));
          lockStore(L);
          localStorage.setItem(LS.wrapped, wrapped);
          /* Η εναλλακτική ταυτότητα της συσκευής μένει συγχρονισμένη με τις
             νέες λέξεις — δεν τη χρησιμοποιεί το kmHead όσο υπάρχει κλειδαριά,
             αλλά δεν αφήνουμε νεκρή τιμή να περιμένει. */
          localStorage.setItem(LS.auth, d.authToken);
          pendingWords = null;
          wordsMode = 'new';
          b.disabled = false; b.textContent = 'Συνέχεια';
      ...
      /* ΤΙΠΟΤΑ τοπικά δεν άλλαξε: η συσκευή δουλεύει με τις ΠΑΛΙΕΣ λέξεις,
         που εξακολουθούν να ισχύουν αφού ο server δεν δέχτηκε τη νέα. */
      stop((err && err.message) ? err.message : 'Δεν έχεις δίκτυο αυτή τη στιγμή. Δοκίμασε ξανά.');
    });
  }
  ...
  function pullNow() {
    ...
    return kmKeyReady().then(function (k) {
      ...
      .then(function () {
        ...
        /* v46 — ΚΑΘΕ ανοιχτή οθόνη που δείχνει αριθμούς ξαναζωγραφίζεται.
           5/9/2026: ο υπολογιστής (ενεργός) έδειχνε 2 εκκρεμή και το κινητό
           (ανάγνωση) 4, ώρες μετά τη διαγραφή. Το κατέβασμα είχε γίνει
           σωστά· η ΟΘΟΝΗ έμενε στην παλιά εικόνα. Δύο συσκευές που δείχνουν
           διαφορετικό νούμερο για τα ίδια δεδομένα είναι χειρότερο από
           καθυστέρηση: ο χρήστης δεν ξέρει ποια να πιστέψει. */
        if (!el('s-menu').hidden)     { renderMenu(); }
        if (!el('s-sup').hidden)      { render('s-sup'); }
        if (!el('s-settings').hidden) { renderSettings(); }
      });
  }
  ...
  /* v62 · Brief Ε §4 #11 (α) — Η ΕΠΙΣΤΡΟΦΗ ΤΟΥ ΔΙΚΤΥΟΥ ΞΕΚΙΝΑΕΙ ΤΗΝ ΟΥΡΑ.
     Εύρημα 11/9: η ουρά σταματούσε στο «χωρίς σύνδεση» και δεν ξαναπροσπαθούσε
     ποτέ — το FAQ «διαβάζονται μόνα τους» ήταν ψέμα χωρίς αυτή τη γραμμή. */
  window.addEventListener('online', function () { schedule(800); });

  /* v62 · Brief Ε §7β — ΜΟΝΙΜΗ ΑΠΟΘΗΚΕΥΣΗ. Εύρημα 11/9: δεν καλούνταν πουθενά,
     άρα ο browser μπορούσε να σβήσει ασυγχρόνιστα τιμολόγια όταν πιέζεται ο
     χώρος. Ζητείται μία φορά· η απάντηση καταγράφεται στο διαγνωστικό, δεν
     μπλοκάρει τίποτα. */
  function askPersist() {
    try {
      if (navigator.storage && navigator.storage.persist) {
        navigator.storage.persisted().then(function (yes) {
          if (yes) { return; }
          navigator.storage.persist().then(function (ok) { try { localStorage.setItem('km_persist', ok ? '1' : '0'); } catch (e) {} });
        });
      }
    } catch (e) {}
  }
  ...
  function instSteps() {
    ...
    if (p === 'ios') {
      /* 🔴 v76 (20/9/2026) — ΔΙΟΡΘΩΣΗ ΞΕΠΕΡΑΣΜΕΝΗΣ ΟΔΗΓΙΑΣ.
         Η v71–v75 έλεγαν «άνοιξέ το στο Safari, όχι Chrome». Από τον
         iOS 16.4 (Μάρτιος 2023) ΚΑΘΕ browser του iPhone μπορεί να βάλει
         εφαρμογή στην αρχική οθόνη, και με σωστό manifest ανοίγει ως
         εφαρμογή. Η επιβεβαίωση: τεκμηρίωση Google, Chrome Help
         «Use web apps — iPhone & iPad». Η παλιά οδηγία έστελνε τον μισό
         κόσμο να αλλάξει browser χωρίς λόγο.
         ⚠ Η πραγματική διαφορά είναι ΠΟΥ είναι το Κοινοποίηση:
         Safari κάτω, Chrome πάνω δεξιά. Αυτό λέει τώρα το βήμα 1. */
      step('Πάτα το <b>Κοινοποίηση</b> — το τετράγωνο με το βέλος προς τα πάνω. Στο <b>Safari</b> είναι <b>κάτω</b> στην οθόνη, στο <b>Chrome</b> <b>πάνω δεξιά</b> δίπλα στη διεύθυνση.');
  ...
  /* 🔴 v74 (20/9/2026, απόφαση Stavros — ανατρέπει τον ρυθμό 1/4/10 της v72):
     Η ΚΑΡΤΑ ΒΓΑΙΝΕΙ ΣΕ ΚΑΘΕ ΑΝΟΙΓΜΑ ΤΗΣ ΣΕΛΙΔΑΣ. Καμία άλλη διαδρομή.

     ΤΟ ΕΠΙΧΕΙΡΗΜΑ (Stavros): κάθε «μόνιμη πόρτα» μέσα στην εφαρμογή
     (γραμμή στο Μενού, ερώτηση στο FAQ) τη βλέπει μόνο όποιος έχει ΗΔΗ
     μπει μέσα — δηλαδή δώρο άχρηστο. Η οδηγία χρειάζεται ΠΡΙΝ κάνει
     οτιδήποτε, στο πρώτο δεύτερολεπτο, αλλιώς δεν τη ζητάει ποτέ.

     ΓΙΑΤΙ ΔΕΝ ΓΙΝΕΤΑΙ ΕΜΠΟΔΙΟ: την κάρτα τη βλέπει ΜΟΝΟ όποιος ΔΕΝ έχει
  ...
  function boot() {
    ...
    /* Πηγή εγκατάστασης και σύσταση — ο κανόνας ζει στο refCaptureSrc(). */
    ...
    /* v48 — ΜΕΤΑ ΤΗΝ ΥΙΟΘΕΤΗΣΗ ΤΗΣ ΚΛΕΙΔΑΡΙΑΣ, ΞΑΝΑ ΚΑΤΕΒΑΣΜΑ.
       Μετρήθηκε 6/9 στον πραγματικό λογαριασμό: δεύτερη συσκευή που δεν έχει
       περάσει ακόμα στη νέα βάση μπαίνει εδώ με ΜΟΝΟ το παλιό κλειδί. Το
       κατέβασμα τρέχει πρώτο, δεν ανοίγει τον φάκελο (είναι πια με το Κ) και
       αφήνει μήνυμα σφάλματος στην οθόνη· η υιοθέτηση γίνεται αμέσως μετά,
  ...
  /* v52 — ΑΛΛΑΓΗ 12 ΛΕΞΕΩΝ. Ίδια πύλη με το «Οι 12 λέξεις μου»: η οθόνη
  ...
  /* ══ v62 · Brief Ε §4 — ΕΡΩΤΗΣΕΙΣ & ΑΠΑΝΤΗΣΕΙΣ ══════════════════════
     Και τα 17 κείμενα (1-16 + 12β) που ενέκρινε ο Stavros 11/9/2026.
     ⚠ v71 (19/9) — ΑΝΤΙΣΤΡΟΦΗ ΤΗΣ ΑΠΟΦΑΣΗΣ ΤΗΣ 16/9. Ως τη v70 το #14 (εγκατάσταση
     σε iPhone) κρατιόταν έξω «μέχρι να γίνει η δοκιμή σε πραγματικό iPhone». Αυτό
     εγγυάτο ΔΕΥΤΕΡΗ δοκιμή: θα έμπαινε μετά κείμενο που κανείς δεν είχε ελέγξει σε
     συσκευή. Τώρα μπαίνει ΠΡΙΝ, ο δοκιμαστής το ΑΚΟΛΟΥΘΕΙ, και η ίδια η δοκιμή το
     επαληθεύει — μία φορά. Ως τη v70 ο χρήστης iPhone διάβαζε οδηγία Chrome (⋮ →
     «Εγκατάσταση εφαρμογής») που στη συσκευή του ΔΕΝ ΥΠΑΡΧΕΙ.
     Το #12 ξαναγράφτηκε 16/9 για την αναστολή 72 ωρών.
     Κάθε 🔗 είναι κουμπί που οδηγεί στην οθόνη — όχι κείμενο. */
  var FAQ = [
    { h: 'Γενικά' },
    ...
    { q: 'Διάβασε λάθος ποσό ή λάθος ημερομηνία. Διορθώνεται;',
      a: 'Ναι, πάντα. Άνοιξε το τιμολόγιο και πάτα «✎ Επεξεργασία στοιχείων». Από εκεί αλλάζεις ποσά, ημερομηνία ή προμηθευτή.' },
    { q: 'Έγραψα λάθος το όνομα ενός προμηθευτή σε πολλά τιμολόγια.',
      a: 'Άνοιξε Προμηθευτές → τον προμηθευτή → «✎ Μετονομασία προμηθευτή». Αλλάζουν όλα τα τιμολόγιά του μαζί. Αν διαλέξεις όνομα που υπάρχει ήδη, οι δύο προμηθευτές ενώνονται. Έχεις 10 δευτερόλεπτα για αναίρεση.',
      go: 's-sup', gt: 'Προμηθευτές' },
    { h: 'Λογαριασμός και ασφάλεια' },
    { q: 'Τι είναι οι 12 λέξεις;',
      a: 'Το κλειδί του λογαριασμού σου. Με αυτές ανοίγεις τα τιμολόγιά σου σε άλλο κινητό. Γράψ’ τες σε χαρτί. Μην τις φωτογραφίσεις και μην τις αντιγράψεις. Εμείς δεν τις έχουμε.' },
    { q: 'Έχασα το χαρτί με τις 12 λέξεις.',
      a: 'Όσο έχεις το κινητό σου, δεν χάνεται τίποτα: τις ξαναβλέπεις στις Ρυθμίσεις → «Οι 12 λέξεις μου», με δακτυλικό ή PIN. Αυτό δουλεύει μόνο αν το κινητό σου έχει κλείδωμα οθόνης, γιατί χωρίς κλείδωμα οι λέξεις δεν εμφανίζονται, για την ασφάλειά σου. Αν χάσεις και το χαρτί και το κινητό, δεν μπορούμε να ανοίξουμε τον φάκελό σου, γιατί δεν έχουμε το κλειδί.',
      go: 's-settings', gt: 'Ρυθμίσεις → Οι 12 λέξεις μου' },
    { q: 'Τι βλέπετε εσείς από τα τιμολόγιά μου;',
      a: 'Τίποτα. Κρατάμε το email σου και έναν σφραγισμένο φάκελο με τα τιμολόγια και τις φωτογραφίες σου. Ο φάκελος κλειδώνει στο κινητό σου πριν φύγει και χωρίς τις 12 λέξεις σου δεν ανοίγει, ούτε από εμάς. Αποθηκεύεται σε διακομιστές στην Ευρωπαϊκή Ένωση.' },
    { q: 'Μπορώ να το έχω σε δύο κινητά;',
      a: 'Ο λογαριασμός δουλεύει σε ένα κινητό τη φορά. Αν μπεις με τις 12 λέξεις σε νέο κινητό, τα τιμολόγια μεταφέρονται εκεί και το παλιό κινητό σταματά να τα δείχνει, ώστε να μην τα βλέπει όποιος το κρατάει.' },
    { q: 'Λειτουργεί χωρίς ίντερνετ;',
      a: 'Ναι. Αν τραβήξεις τιμολόγιο χωρίς δίκτυο, αποθηκεύεται στο κινητό. Μόλις επιστρέψει το δίκτυο, συγχρονίζεται και, αν έχεις κλειδί Gemini, διαβάζονται μόνα τους τα ποσά. Εσύ απλώς τα επιβεβαιώνεις.' },
    { q: 'Πώς διαγράφω τον λογαριασμό μου;',
      a: 'Ρυθμίσεις → «Διαγραφή λογαριασμού». Η διαγραφή <b>δεν γίνεται αμέσως: γίνεται σε 72 ώρες</b>. Μόλις τη ζητήσεις, ο λογαριασμός παγώνει και σου στέλνουμε email. Αν δεν τη ζήτησες εσύ, τη σταματάς με τις 12 λέξεις σου από οποιαδήποτε συσκευή. Μετά τις 72 ώρες σβήνουμε τον φάκελό σου από τους διακομιστές μας και δεν υπάρχει επαναφορά, ούτε με τις 12 λέξεις.',
      go: 's-settings', gt: 'Ρυθμίσεις' },
    { q: 'Αν κάποιος πάρει το κινητό μου;',
      a: 'Αν το κινητό είναι ξεκλείδωτο, βλέπει τα τιμολόγια, όπως θα έβλεπε και τις φωτογραφίες σου. Τα τιμολόγια στη συσκευή τα προστατεύει το κλείδωμα οθόνης του κινητού σου, γι’ αυτό βάλε κλείδωμα. Η διαγραφή του λογαριασμού και οι 12 λέξεις ζητούν ξανά δακτυλικό ή PIN — και αν ζητηθεί διαγραφή, έχεις 72 ώρες να τη σταματήσεις με τις 12 λέξεις σου.' },
    { h: 'Εγκατάσταση' },
    { id: 'install-android',
      q: 'Πώς το βάζω στην αρχική οθόνη σαν εφαρμογή; (Android)',
      a: 'Στο <b>Chrome</b>, πάτα το μενού (⋮) και διάλεξε <b>«Εγκατάσταση εφαρμογής»</b>, όχι «Προσθήκη στην αρχική οθόνη». Η εγκατάσταση ανοίγει σαν κανονική εφαρμογή, χωρίς τη γραμμή διευθύνσεων.' },
    /* id: ΣΗΜΑΝΣΗ ΓΙΑ ΤΟΝ ΕΛΕΓΧΟ ΤΟΥ deploy (.bat), σε ΛΑΤΙΝΙΚΟΥΣ — το findstr
       μέσα σε .bat ΔΕΝ διαβάζει ελληνικά (μετρήθηκε 19/9/2026, ο έλεγχος της
       v70 απέτυχε ενώ ο κώδικας ήταν σωστός). Δεν το διαβάζει η renderFaq. */
    { id: 'install-ios',
      q: 'Πώς το βάζω στην αρχική οθόνη σαν εφαρμογή; (iPhone)',
      a: 'Πάτα το κουμπί <b>Κοινοποίηση</b> — το τετράγωνο με το βέλος προς τα πάνω. Στο <b>Safari</b> είναι κάτω στην οθόνη, στο <b>Chrome</b> πάνω δεξιά δίπλα στη διεύθυνση — και οι δύο δουλεύουν, κύλησε προς τα κάτω και διάλεξε <b>«Προσθήκη στην οθόνη Αφετηρίας»</b>, και μετά <b>«Προσθήκη»</b>.<br><br>Μετά κλείσε το Safari και άνοιξε την εφαρμογή <b>από το εικονίδιο</b>: ανοίγει σε ολόκληρη την οθόνη, χωρίς τη γραμμή διευθύνσεων. Το iPhone δεν έχει «Εγκατάσταση εφαρμογής» όπως το Android — αυτός είναι ο δρόμος.' },
    { h: 'Επικοινωνία και επόμενα' },
    ...
    { q: 'Πώς επικοινωνώ μαζί σας;',
      a: 'Από το Μενού → «Υποστήριξη». Ανοίγει email προς εμάς, με έτοιμο αριθμό εισιτηρίου για να βρίσκουμε γρήγορα τη συζήτησή σας. Για ιδέες και παρατηρήσεις, «Η γνώμη σου».',
  ...
  function renderFaq() {
    ...
    faqDone = true;
    var h = '';
    FAQ.forEach(function (x, i) {
      if (x.h) { h += '<div class="faq-h">' + x.h + '</div>'; return; }
      h += '<details class="faq"><summary>' + x.q + '</summary><div class="a">' + x.a +
           (x.go ? '<br><button type="button" class="go" data-go="' + x.go + '">🔗 ' + x.gt + '</button>' : '') +
           '</div></details>';
    });
    el('faq-body').innerHTML = h;
    el('faq-body').querySelectorAll('.go').forEach(function (b) { b.onclick = function () { goto(b.getAttribute('data-go')); }; });

    /* v63 · Μία ερώτηση ανοιχτή τη φορά (Stavros, 16/9): όταν ανοίγει νέα,
       κλείνει η προηγούμενη. Σε οθόνη τηλεφώνου δύο ανοιχτές απαντήσεις
       σπρώχνουν τη δεύτερη εκτός οθόνης και φαίνεται σαν να μην άνοιξε.
       ⚠ Το συμβάν `toggle` ΔΕΝ ανεβαίνει (δεν κάνει bubble) — δεν πιάνεται με
         έναν ακροατή στον γονέα. Μπαίνει σε κάθε <details> ξεχωριστά.
       ⚠ ΟΧΙ το πρόσφατο χαρακτηριστικό name="" των <details>: δεν το έχουν
         όλα τα κινητά που θα τρέξουν την εφαρμογή. */
    var all = el('faq-body').querySelectorAll('details.faq');
  ...
  /* ══ v62 · Brief Ε §3 — Η ΓΝΩΜΗ ΣΟΥ ═════════════════════════════════
     Ο server (POST /api/km/feedback, 14/9) δεν παίρνει folder_id ούτε IP· το
     email μόνο με «Θέλω απάντηση». Το install_id πάει ΜΟΝΟ για το φρένο των
     5 ανά ημέρα και στον server γίνεται sha256(install_id + ημέρα). */
  var fbStars = 0;
  function renderFb() {
    fbStars = 0; paintStars();
    el('fb-text').value = '';
    el('fb-reply').checked = false;
    el('fb-err').hidden = true; el('fb-ok').hidden = true;
    el('fb-send').disabled = false; el('fb-send').textContent = 'Στείλε';
  }
  function paintStars() {
    el('fb-stars').querySelectorAll('button').forEach(function (b) {
      b.className = Number(b.getAttribute('data-s')) <= fbStars ? 'on' : '';
    });
  }
  el('fb-stars').querySelectorAll('button').forEach(function (b) {
    b.onclick = function () { fbStars = Number(b.getAttribute('data-s')); paintStars(); };
  });
  el('fb-send').onclick = function () {
    var e = el('fb-err'), b = el('fb-send');
    e.hidden = true;
    var text = (el('fb-text').value || '').trim();
    if (!fbStars && !text) { e.textContent = 'Διάλεξε αστέρια ή γράψε δύο λόγια — ένα από τα δύο αρκεί.'; e.hidden = false; return; }
    var reply = el('fb-reply').checked;
    var email = localStorage.getItem(LS.email) || '';
    if (reply && !email) { e.textContent = 'Δεν έχεις email στη συσκευή — ξετσέκαρε το «Θέλω απάντηση» ή γράψε μας από την Υποστήριξη.'; e.hidden = false; return; }
    b.disabled = true; b.textContent = 'Στέλνεται…';
    fetch(KM_API + 'feedback', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ install_id: localStorage.getItem(LS.id) || '', stars: fbStars || null, text: text, ver: shortVer(APP_VER), reply: reply, email: reply ? email : null })
    }).then(function (r) { return r.json().catch(function () { return {}; }).then(function (j) { return { s: r.status, j: j }; }); })
      .then(function (x) {
        if (x.s === 429) { throw new Error('Έφτασες το όριο για σήμερα (5 μηνύματα). Ξαναδοκίμασε αύριο.'); }
        if (x.s !== 200 || !x.j.ok) { throw new Error('Δεν έφτασε (σφάλμα ' + x.s + '). Δοκίμασε ξανά.'); }
        el('fb-ok').hidden = false; b.textContent = 'Στάλθηκε';
      })
      .catch(function (err) { b.disabled = false; b.textContent = 'Στείλε'; e.textContent = err.message || 'Χωρίς σύνδεση. Δοκίμασε ξανά.'; e.hidden = false; });
  };
  ...
  /* Η.11β — ΑΚΥΡΩΣΗ ΑΠΟ ΤΗΝ ΙΔΙΑ ΤΗ ΣΥΣΚΕΥΗ.
     ...
     Η σελίδα fastwrite.tech/akyrosi είναι η άλλη πόρτα, για
  ...
  el('dp-cancel').onclick = function () {
    ...
    kmFetch('delete/cancel', { method: 'POST', headers: kmHead(), body: JSON.stringify({ device_name: devName() }) })
      .then(function (r) {
        ...
        if (r.status === 403) { stop('Ο κωδικός αυτής της συσκευής δεν ισχύει πια. Ακύρωσε από τη σελίδα fastwrite.tech/akyrosi με τις 12 λέξεις σου.'); return null; }
  ...
  /* v46 — Ως τη v45 το κατέβασμα γινόταν ΜΟΝΟ όταν η εφαρμογή ερχόταν στο
     προσκήνιο. Συσκευή ανάγνωσης αφημένη ανοιχτή στην οθόνη έμενε στην
     παλιά εικόνα επ' αόριστον — και αυτή ακριβώς είναι η χρήση της. */
  ...
  function updToast() {
    ...
    wait.then(function () {
      ...
      /* v90 · KM-UPD-7S — 26/9, εύρημα Stavros στο κινητό: στα 3″ φαινόταν αλλά
         δεν προλάβαινε να διαβαστεί. 7″, και ✕ για όποιον το θέλει νωρίτερα. */
      setTimeout(hide, UPD_MS);
    });
  }
  ...
  if ('serviceWorker' in navigator) {
    ...
    /* 🔴 v77 (20/9/2026) — Η ΕΓΓΡΑΦΗ ΦΕΡΝΕΙ ΤΗΝ ΕΚΔΟΣΗ ΜΕΣΑ ΣΤΗ ΔΙΕΥΘΥΝΣΗ.
       ΜΕΤΡΗΘΗΚΕ 20/9: μετά το deploy της v76, το `version.json` και το `app.js`
       ήρθαν σωστά αλλά το `sw.js` σερβιρόταν ακόμα ως `km-v75` με
       `cf-cache-status: HIT` — τρεις μετρήσεις σε δέκα λεπτά, παρότι το
       `_headers` δίνει `no-cache` σε όλο το /kostometro/*.

       ΤΙ ΣΗΜΑΙΝΕΙ ΑΥΤΟ: ο worker της συσκευής ΔΕΝ αντικαθίσταται ποτέ,
       και η μνήμη του κρατάει ΠΑΛΙΟ `index.html`. Με αργό δίκτυο το
       φρένο των 2,5″ σερβίρει αυτό το παλιό. Έτσι στο tablet του Stavros
       δεν εμφανίστηκαν οι σύνδεσμοι των Όρων, ενώ υπάρχουν στον server.

       Η ΛΥΣΗ: κάθε έκδοση ζητάει ΑΛΛΗ διεύθυνση (`sw.js?v=v77`). Η άκρη
       δεν έχει τι να κρατήσει για μια διεύθυνση που δεν είχε ξαναδεί.
       ⚠ Το query ΔΕΝ αλλάζει το scope — αυτό βγαίνει από τη διαδρομή.
       ⚠ ΔΕΝ εξαρτάται από χειροκίνητο purge στο Cloudflare: αυτοδιορθώνεται
       σε κάθε έκδοση, για πάντα. KM-SW-VERSIONED-URL */
    navigator.serviceWorker.register('/kostometro/sw.js?v=' + shortVer(APP_VER)).then(function (reg) {
      var check = function () {

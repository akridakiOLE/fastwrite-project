...
echo.
echo  v79 = YPERFORTOMENO MONTELO - TO EPOMENO, AMESOS
echo.
echo        METRITHIKE 22/9 20:34 me ti v78: 503 "high demand" sto
echo        gemini-3.6-flash, ena lepto afou to idio kleidi diavase se 2,7".
echo        To fortio einai ana MONTELO, oxi ana kleidi.
echo.
echo        TI ALLAZEI: sto 5xx dokimazetai AMESOS to epomeno montelo tis
echo        listas, me to idio kleidi. Ola yperfortomena = i v78 xanadokimazei.
echo.
...
if errorlevel 1 goto nm
findstr /c:"v79" site\kostometro\app.js >nul
if errorlevel 1 goto nm
echo    OK - 11 deiktes.

echo [2/5] Deiktes pou PREPEI na leipoun...
findstr /c:"km-v78" site\kostometro\sw.js >nul
if not errorlevel 1 goto lo
findstr /c:"\"v\": \"v78\"" site\kostometro\version.json >nul
if not errorlevel 1 goto lo
...
echo [4/5] Souites + 80 apodeixeis metallaxis, MIA TI FORA...
...
if errorlevel 1 goto failed
git add deploy_v79.bat commit_v79.txt
if errorlevel 1 goto failed
git commit -F commit_v79.txt
if errorlevel 1 goto failed
...
echo   1) Anoixe to Kostometro, Menou kato: v79
echo   2) Travixe timologia. Sti "Diarkeia anagnosis" fainetai POIO
echo      montelo diavase - an to 3.6 itan yperfortomeno, tha grafei allo.
echo.
echo ================================
echo.
exit /b 0
...
exit /b 1
:lo
echo    Epezise PALIOS deiktis (km-v78, v78, 30000, palio generationConfig i sw.js xoris ekdosi).
goto failed
:nm

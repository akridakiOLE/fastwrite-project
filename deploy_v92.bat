...
echo.
echo  v92 = ARITHMOS AITIMATOS APO TON SERVER + "SYNEXEIA SE..." + AYTOMATI APANTISI support@
echo.
echo        Metavasi vasis: schema\km_support.sql (CREATE IF NOT EXISTS).
echo        Neo mystiko Cloudflare: KM_SUPPORT_KEY (mono gia to Apps Script).
echo.
...
if errorlevel 1 goto nm
findstr /c:"\"v\": \"v92\"" site\kostometro\version.json >nul
if errorlevel 1 goto nm
...
findstr /c:"km-v91" site\kostometro\sw.js >nul
...
echo [6/7] Mystiko KM_SUPPORT_KEY...
if not exist "secrets" mkdir secrets
if not exist "secrets\km_support_key.txt" (
...
if errorlevel 1 goto failed
echo    OK - to kleidi zei MONO sto secrets\km_support_key.txt (ektos git)
...
if errorlevel 1 goto failed
git add src\km.js schema\km_support.sql tools\support\support_autoreply.gs
if errorlevel 1 goto failed
git add tests\v92.test.mjs tests\h11b.test.mjs tests\pinakas.test.mjs tests\ref.test.mjs tests\v88.test.mjs tests\verify.test.mjs tests\leads.test.mjs tests\cleanup.test.mjs tests\menu62.spec.js
if errorlevel 1 goto failed
git add deploy_v92.bat commit_v92.txt
if errorlevel 1 goto failed
git commit -F commit_v92.txt
if errorlevel 1 goto failed
...
exit /b 1
:lo
echo    Epezise PALIOS deiktis (km-v91, newTicket, AIza).
goto failed
:nm
